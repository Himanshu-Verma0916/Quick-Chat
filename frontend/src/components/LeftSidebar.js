...
const LeftSidebar = () => {
  const {users, fetchUsers, selectedUser, setSelectedUser,unseenMessages,setUnseenMessages} = useContext(ChatContext);
  console.log(users);
  ...
  useEffect(()=>{
    console.warn(filteredUser);
  },[filteredUser]);

  const navigate = useNavigate();
  return (
    <div className={`bg-[#818582]/10 h-full p-5 rounded-r-xl overflow-y-hidden text-white ${selectedUser ? 'max-md:hidden' : ''}`}>
      <div className='pb-5'>
        ...
        <div className='bg-[#282842] rounded-full flex items-center  px-4 gap-2 mt-5'>
          <img src={assets.search_icon} alt='search' className='max-h-5 mt-3 mb-2' />
