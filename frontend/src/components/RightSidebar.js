    ...
    <div className={`bg-[#8185B2]/10 text-white w-full relative overflow-y-scroll ${selectedUser ? 'max-md:hidden' : ''} `}>
      <div className='pt-16 flex flex-col items-center gap-2 text-xs font-light mx-auto '>
        ...
        <h1 className='px-10 text-xl font-medium mx-auto flex items-center gap-2 whitespace-nowrap'>
          {onlineUsers.includes(selectedUser._id) &&<p className='w-2 h-2 rounded-full bg-green-500 font-medium mx-auto flex items-center gap-2'></p>}
          {
        ...
        <div className='mt-2 max-h-[200px] overflow-y-scroll grid grid-cols-2 gap-4 opacity-80'>
          {
            msgImages.map((imgUrl, index) => (
              <div key={index} onClick={() => window.open(imgUrl)} className='cursor-pointer rounded'>
                <img src={imgUrl} className='h-full rounded-md' />
              </div>
            ))
          }
        </div>
